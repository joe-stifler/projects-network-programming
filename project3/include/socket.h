...
namespace sock {
    ...
    int ReadSocket(int sockfd, char *recvline, int maxline) {
        int numBytes;

        /* 
            Lemos o 32 bytes que representarão o tamanho da mensagem enviada pelo servidor.
            Verificamos também se houve erro na leitura.
        */
        if (read(sockfd, (char *) &numBytes, sizeof(int)) < 0) {
            perror("read error");
            exit(1);
        }

        /*
            Dado que já sabemos o tamanho mensagem, basta que a lemos.
            Verificamos neste processo se houve erros na leitura.
        */
        if (numBytes > 0) {
            if (read(sockfd, recvline, numBytes) < 0) {
                perror("read error");
                exit(1);
            }
    ...
    void FetchCommands(int sockfd, char *recvline, int maxline) {
        while (true) {
            int numBytes = ReadSocket(sockfd, recvline, maxline);

            if (numBytes > 0) printf("Commando a ser executado: '%s'\n", recvline);

            /* 
                Verificamos se o comando passado pelo servidor não é o de término
                Caso seja, saímos do loop    
            */
            if (strcmp(recvline, END_COMMAND) == 0 || numBytes == 0) break;

            /*
                No comando que será executado, concatenamos o redirecionamento
                da saída do comando para o arquivo out.out
            */
            strcat(recvline, " > out.out");

            /* Executamos o comando passado pelo servidor */
            if(system(recvline) != 0);

            /* Abrimos o arquivo que contém o output da execução do comando */
            FILE *fp = fopen("out.out", "r");

            /* Obtemos o tamanho (em bytes) deste arquivo */
            fseek(fp, 0, SEEK_END); // seek to end of file
            numBytes = MIN(maxline, ftell(fp));
            fseek(fp, 0, SEEK_SET);

            /* Lemos estes bytes para o buffer recvline */
            if (fread(recvline, sizeof(char), numBytes, fp) != (size_t) numBytes) {
                perror("read error");
                exit(1);
            }
            ...
            /* escrevemos no socket o número de bytes lidos do arquivo */
            sock::Write(sockfd, (char *) &numBytes, sizeof(int));

            /* Escrevemos no socket os bytes lidos do arquivo */
            sock::Write(sockfd, recvline, numBytes);
        }
    }
}
